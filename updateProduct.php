<?php include 'header.php' ;

$product   = mysqli_query($conn, "SELECT * FROM printer WHERE no = '".$_GET['id']."'");
if(mysqli_num_rows($product) == 0){
    echo "<script>window.location ='dataProduct.php'</script>";
}
$p          = mysqli_fetch_object($product);
?>

<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Update Product</h1>
        <a class="btn btn-primary" href="dataProduct.php" role="button">Back</a>
      </div>

      <form action="" method="POST">
    <div class="mb-3">
    <label for="exampleInputEmail1" class="form-label">Name Product</label>
    <input type="text" value="<?= $p->nam2e_product ?>" name="name" require class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
  </div>

  <div class="mb-3">
    <label for="exampleInputEmail1" class="form-label">Color</label>
    <input type="text" value="<?= $p->color ?>" name="color" require class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
  </div>

  <div class="mb-3">
    <label for="exampleInputEmail1" class="form-label">Count</label>
    <input type="text" value="<?= $p->count ?>" name="count" require class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
  </div>

  

  <button type="submit" name="submit" class="btn btn-primary">Updated</button>
</form>

<?php
                        if(isset($_POST['submit'])){

                            $name = $_POST['name'];
                            $color = $_POST['color'];
                            $count = $_POST['count'];   

                                    $query = "UPDATE printer SET
                                    name_product = '".$name."',
                                    color = '".$color."',
                                    count = '".$count."'
                                    WHERE no = '".$_GET['id']."'
                               ";
                                    $simpan = mysqli_query($conn, $query);
                                    if($simpan){
                                        echo "<script>window.location='dataProduct.php'</script>";
                                    }else{
                                        echo 'gagal disimpan',mysqli_error($conn);
                                    }
                            }

                        
                    ?>
    </main>
    <?php include 'footer.php' ?>